...
const LineChart = ({ stockSeriesData }) => {
  const formattedData1 = Object.entries(
    stockSeriesData["Time Series (Daily)"]
  ).map(([date, info]) => ({
    x: new Date(date),
    y: parseFloat(info["1. open"]),
  }));
  const formattedData2 = Object.entries(
    stockSeriesData["Time Series (Daily)"]
  ).map(([date, info]) => ({
    x: new Date(date),
    y: parseFloat(info["2. high"]),
  }));
  const formattedData3 = Object.entries(
    stockSeriesData["Time Series (Daily)"]
  ).map(([date, info]) => ({
    x: new Date(date),
    y: parseFloat(info["3. low"]),
  }));
  const formattedData4 = Object.entries(
    stockSeriesData["Time Series (Daily)"]
  ).map(([date, info]) => ({
    x: new Date(date),
